...
The key thing to understand is that this calculation requires a **reference point**: the pressure at sea level ($P_0$).

$Altitude = 44330 * (1 - (\frac{CurrentPressure}{SeaLevelPressure}) ^{0.1903})$

The `bmp.get_altitude()` function uses a standard, internationally agreed-upon value for sea level pressure: $101325$ Pascals. This is why the output is an "approximate" altitude. The actual local sea level pressure changes with the weather, so the altitude reading can fluctuate from day to day even if the sensor doesn't move.
